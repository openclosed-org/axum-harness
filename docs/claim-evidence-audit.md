...
| Runtime topology can grow toward single-VPS and K3s-style deployments | declared / checked | checked when platform gates run | `platform/model/**`, `infra/k3s/**`, `just validate-platform`, `just validate-topology` | Platform metadata and manifests are not operational proof of staging/production readiness. |
...
| K3s/GitOps paths are delivery landing zones | partial | declared / checked | `infra/k3s/**`, `infra/gitops/**`, `platform/model/**` | Health, promotion, rollback, and drift behavior remain not proven. |
